...
pub fn start_sshd(allocator: std.mem.Allocator, port: i32) !void {
    ...
    if(!try checkDecrypt(allocator, session)) {
...
fn checkDecrypt(allocator: std.mem.Allocator, session: *c.ssh_session_struct) !bool {
    var msg: c.ssh_message = undefined;

    const name = "Welcome to dols, v0.0.0\n";
    const instructionFormat = "Enter disk decryption key for";
    var prompt: [1][*c]const u8 = .{@ptrCast("Password: ")};
    var echo = [1]u8{0};
